	...
	while wait() do
		pcall(function()
			if BringMob then
                local NUMS = 0
                local OthersMon =  0
                AOthersMon = AOthersMon or 10
                limit = limit or 1/0
                local Enemies = workspace.Enemies:GetChildren()
			    for i=1,#Enemies do local v = Enemies[i]
                    if limit and NUMS >= limit then end
                    if not v:FindFirstChild("HumanoidRootPart") then end
                    if v:FindFirstChildOfClass("Humanoid") and v.Humanoid.Health > 0  and dist(v.HumanoidRootPart.Position) <= 5000 and not v.Name:lower():find("boss") then
                        local IsVaidMon = not Mon or (v ~= Mon and v.Name == Mon.Name)
                        if IsVaidMon then
                            if syn and InMyNetWork(v.HumanoidRootPart) or (v.HumanoidRootPart.Position - Client.Character.HumanoidRootPart.Position).Magnitude <= 300 then
                                if IsVaidMon or OthersMon < AOthersMon then
                                    ...
                                    if IsVaidMon then
                                        NUMS = NUMS + 1
                                    elseif OthersMon < AOthersMon then
                                        OthersMon = OthersMon + 1
                                    end
                                end
                            end
                        end
