...
MasteryRight:AddSeperator("Item")
...
local SelectMaterial = MasteryRight:AddDropdown("Select Material", MaterialMethod, function(Value)
	_G.SelectMaterial = Value
end)

MasteryRight:AddToggle("Auto Farm Material", _G.AutoFarmMaterial, function(Value)
...
MasteryRight:AddLine()

MasteryRight:AddToggle("Auto Shark Saw", _G.AutoSharkSaw, function(Value)
...
MasteryRight:AddToggle("Auto Saber", _G.AutoSaber, function(Value)
    _G.AutoSaber = Value
    if Value == false then
        wait(0.3)
        TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
MasteryRight:AddToggle("Auto Trident", _G.AutoTrident, function(Value)
...
MasteryRight:AddToggle("Auto Pole", _G.AutoPole, function(Value)
    _G.AutoPole = Value
    if Value == false then
        wait(0.3)
        TP(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame)
...
MasteryRight:AddToggle("Auto Bisento V2", _G.AutoBisento, function(Value)
                        ...
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v.Name == "Greybeard" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                repeat wait()
                                    ...
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                until not _G.AutoBisento or v.Humanoid.Health <= 0 or not v.Parent
                                UnEquip()
                                FastAttack = false
                            end
                        end
