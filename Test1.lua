...
spawn(function()
	pcall(function()
		game:GetService("RunService").Stepped:Connect(function()
			if  
			_G.AutoFarmLevel or _G.AutoSecondSea or _G.AutoFarmChests or _G.MobAura or
			...
			_G.AutoFarmDevilFruitMastery or _G.AutoFarmGunMastery or _G.AutoFarmObservation or
			_G.NoClip then
...
CombatLeft:AddButton("Pirate Starter",function()
    TP2(CFrame.new(1027.58875, 19.3245106, 1367.28564, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Middle Town",function()
    TP2(CFrame.new(-582.941345, 6.70024586, 1713.93726, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Jungle",function()
    TP2(CFrame.new(-1255.14795, 11.7000084, 349.906677, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Pirate Village",function()
    TP2(CFrame.new(-1120.14856, 4.60001373, 3855.31763, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Desert",function()
    TP2(CFrame.new(1094.14587, 5.28642607, 4192.88721, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Frozen Village",function()
    TP2(CFrame.new(1185.63379, 7.15136719, -1143.36987, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Marine Fortless",function()
    TP2(CFrame.new(-4697.32324, 20.1976337, 3975.07129, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Skylands 1",function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-4608, 873, -1668))
    TP2(CFrame.new(-4967.83691, 718.535278, -2623.84326, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Skylands 2",function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-7947, 5547, -319))
end)
CombatLeft:AddButton("Prison",function()
    TP2(CFrame.new(5032.81885, 2.3914094, 766.972168, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Colosseum",function()
    TP2(CFrame.new(-1332.94006, 6.35386896, -2896.56055, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Magma Village",function()
    TP2(CFrame.new(-5193.35498, 5.43863821, 8569.56348, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
CombatLeft:AddButton("Whirlpoll",function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(3905, 14, -1941))
end)
CombatLeft:AddButton("Fountain City",function()
    TP2(CFrame.new(5091.09619, 1.06925201, 4033.5708, 1, 0, 0, 0, 1, 0, 0, 0, 1))
end)
...
VisualsRight:AddToggle("Players ESP",_G.PlayersESP ,function(Value)
    _G.PlayersESP = Value
...
VisualsRight:AddToggle("Devil Fruits ESP",_G.DevilFruitsESP,function(Value)
		...
		pcall(function()
			if string.find(v.Name, "Fruit") then
				if _G.DevilFruitsESP then
					if string.find(v.Name, "Fruit") then
						if not v.Handle:FindFirstChild("DevilFruitsESPESP"..Number) then
							local Bb = Instance.new("BillboardGui", v.Handle)
							Bb.Name = "DevilFruitsESPESP"..Number
							Bb.ExtentsOffset = Vector3.new(0, 1, 0)
							Bb.Size = UDim2.new(1, 200, 1, 30)
							Bb.Adornee = v.Handle
							Bb.AlwaysOnTop = true
							...
							v.Handle["DevilFruitsESPESP"..Number].TextLabel.Text = v.Name.."\n"..math.round((v.Handle.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude/3).." m."
						end
					end
				else
					if v.Handle:FindFirstChild("DevilFruitsESPESP"..Number) then
						v.Handle:FindFirstChild("DevilFruitsESPESP"..Number):Destroy()
					end
				end
			end
		end)
...
VisualsRight:AddToggle("Chest ESP",_G.ChestESP,function(Value)
    _G.ChestESP = Value
...
function UpdateChest()
	for i,v in pairs(game.Workspace:GetChildren()) do
		pcall(function()
			if v.Name == "Chest 1" or v.Name == "Chest 2" or v.Name == "Chest 3" then
				if _G.ChestESP then
					if (v.Name == "Chest 1" or v.Name == "Chest 2" or v.Name == "Chest 3") and (v.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 60000 then
