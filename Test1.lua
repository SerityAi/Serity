		...
		pcall(function()
			while wait(.1) do
				if _G.AutoStoredFruits then
					if game:GetService("Players").LocalPlayer.Backpack:FindFirstChild(DevilFruitList) then
						game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StoreFruit", DevilFruitTable, game:GetService("Players").LocalPlayer.Character:FindFirstChild(DevilFruitList))
					end
				end
			end
		end)
...
MiscLeft:AddSeperator("Job ids")

MiscLeft:AddTextbox("Job ids","", JoinJobID,function(value)
	JoinJobID = Value
end)

MiscLeft:AddButton("Join Job ids",function()
