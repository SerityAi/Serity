...
function CheckQuest()
	...
	if MyLevel == 1 or MyLevel <= 10 then -- Bandit [Lv. 5]
		Mon= "Bandit"
		Quest = "BanditQuest1"
		LevelQuest = 1
		CFrameMon = CFrame.new(914, 167, 1495)
		CFrameQuest = CFrame.new(1060, 16, 1549)

	elseif MyLevel == 11 or MyLevel <= 119 then -- Gorilla [Lv. 20]
        if _G.AutoFarmLevel then
            pcall(function()
                if game:GetService("Workspace").Enemies:FindFirstChild("God's Guard") then
                    for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v.Name == "God's Guard" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            repeat wait()
                                Haki()
                                EquipWeapon()
                    ...
                    local Distance = (Vector3.new(4608, 873, -1669) - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
                    if Distance > 3000 then
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(4608, 873, -1669))
                    end
                    TP(CFrame.new(-4848, 994, -2061))
                end
            end)
        end
	...
	elseif MyLevel == 120 or MyLevel <= 149 then -- Chief Petty Officer [Lv. 120]
		...
		Quest = "MarineQuest2"
		LevelQuest = 1
		CFrameMon = CFrame.new(-5008, 170, 3924)
		CFrameQuest = CFrame.new(-5037, 29, 4323)

	elseif MyLevel == 150 or MyLevel <= 174 then -- Sky Bandit [Lv. 150]
		Mon = "Sky Bandit"
		Quest = "SkyQuest"
		LevelQuest = 1
		CFrameMon = CFrame.new(-5094, 428, -2966)
		CFrameQuest = CFrame.new(-4839, 718, -2618)

        local Distance = (Vector3.new(-4839, 718, -2618) - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
        if Distance > 3000 then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(4608, 873, -1669))  -- Bypass Sky 1 Island
        end
	...
	elseif MyLevel == 175 or MyLevel <= 189 then -- Dark Master [Lv. 175]
		...
		Quest = "SkyQuest"
		LevelQuest = 2
		CFrameMon = CFrame.new(-5238, 540, -2126)
		CFrameQuest = CFrame.new(-4839, 718, -2618)

        local Distance = (Vector3.new(-4839, 718, -2618) - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude  -- Bypass Sky 1 Island
        if Distance > 3000 then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(4608, 873, -1669))
        end
	...
	elseif MyLevel == 190 or MyLevel <= 209 then -- Prisoner [Lv. 190]
		...
		Quest = "PrisonerQuest"
		LevelQuest = 1
		CFrameMon = CFrame.new(50878, 150, 568)
		CFrameQuest = CFrame.new(5191, 4, 687)

        local Distance = (Vector3.new(5191, 4, 687) - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude  -- Bypass Sky 1 Island
        if Distance > 3000 then
	...
	elseif MyLevel == 210 or MyLevel <= 249 then -- Dangerous Prisoner [Lv. 210]
		...
		Quest = "PrisonerQuest"
		LevelQuest = 2
		CFrameMon = CFrame.new(5590, 150, 974)
		CFrameQuest = CFrame.new(5191, 4, 687)
	...
	elseif MyLevel == 250 or MyLevel <= 299 then -- Toga Warrior [Lv. 250]
		...
		LevelQuest = 1
		CFrameMon = CFrame.new(-2153, 158, -2871)
		CFrameQuest = CFrame.new(-1581, 7, -2987)
...
function EquipWeapon()
    if _G.SelectWeapon == Value or _G.SelectWeapon == nil or _G.SelectWeapon == {} then
        Melee()
    elseif _G.SelectWeapon == "Melee" then
		Melee()
...
local Tabs = {
    General = Window:AddTab({ Title = "General Tab", Icon = "rbxassetid://16637685852" }),
...
do
    ...
    local Toggle = Tabs.General:AddToggle("MyToggle", {Title = "Auto Farm Level", Default = false })
